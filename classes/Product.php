...
class Product extends Base
{
    ...
    public function card()
    {
        ...
        foreach ($this->getExtraWarranties() as $extraWarranty){
            $productHtml.='<a style="color:white" href="addToCartProduct.php?id='.$extraWarranty->getId().'&parentId='.$this->getId().'" class="btn btn-primary">+ '.$extraWarranty->name.'('.$extraWarranty->getFinalPrice().' RON)</a>';
        }

        $productHtml.='<a href="addToCartProduct.php?id='.$this->getId().'" class="btn btn-primary" style="color:white">Adauga in cos</a>';
        if (getAuthUser() && getAuthAdmin()){
            $productHtml.='<a href="updateProduct.php?id='.$this->getId().'" class="btn btn-success" style="color:white">Edit</a>
                        <a href="deleteProduct.php?id='.$this->getId().'" class="btn btn-danger" style="color:white">Delete</a>';
        }
        $productHtml.='</div>
                </div>
        </div>';
